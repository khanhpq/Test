...
static AFX_EXTENSION_MODULE xopencvDLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("xopencv.DLL Initializing!\n");
		
		// 拡張 DLL を 1 回だけ初期化します。
		if (!AfxInitExtensionModule(xopencvDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(xopencvDLL);

	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("xopencv.DLL Terminating!\n");

		// デストラクターが呼び出される前にライブラリを終了します
		AfxTermExtensionModule(xopencvDLL);
	}
	return 1;   // OK
}
